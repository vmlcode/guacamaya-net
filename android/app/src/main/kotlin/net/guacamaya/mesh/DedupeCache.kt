...
class DedupeCache(
    ...
    fun admit(nodeId: ByteArray, msgId: Int): Boolean {
        val key = Key(nodeId.toIntKey(), msgId)
        val ts = now()
        synchronized(lock) {
            val prev = seen[key]
    ...
    internal fun seed(nodeId: ByteArray, msgId: Int, ts: Long) {
        synchronized(lock) {
            seen[Key(nodeId.toIntKey(), msgId)] = ts
        }
    }

    private data class Key(val nodeId: Int, val msgId: Int)

    private fun ByteArray.toIntKey(): Int {
        var out = 0
        for (i in 0 until min(size, 4)) {
            out = (out shl 8) or (this[i].toInt() and 0xFF)
        }
        return out
    }

    companion object {
        const val DEFAULT_CAPACITY = 65_536
        const val DEFAULT_TTL_MILLIS = 15 * 60 * 1000L   // 15 min flood horizon
    }
}
