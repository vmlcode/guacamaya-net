...
- ✅ Tests unitarios verdes (codec, crypto, geo/compass/proximity).
...
- [ ] **`IngestClient` en Kotlin** (próxima tarea de código): juntar frames recibidos y `POST /ingest`
      al recuperar internet. **Bloqueador conocido:** `MessageEntity` **no persiste la firma de 64 B**
      → hay que añadir columna `sig` (migración Room) antes de poder reconstruir el frame de 118 B. No
      hay cliente HTTP ni WorkManager en las deps todavía. Contrato en [[Backend Data-Mule]].
