...
## Repository structure — two products across branches

This repo holds **two separate products** on different branch lineages. They branch from the same
root commit but share no code; don't try to merge one into the other.

| Branches | Product | Stack | Role |
|---|---|---|---|
| `init-sosnet`, `BTE-comunicacion` | **SOSNet** (this branch) | Native Android, Kotlin + Compose | The primary app: the connectionless BLE/Wi-Fi-Aware mesh described below. |
| `develop`, `BR-01-Backend` | **Guacamaya Red backend** | Bun + TypeScript (Fastify, `@noble/ed25519`, Supabase) | Optional **data-mule ingestion point** for mesh reports. The Expo app that was also on `develop` is **dropped** — the Kotlin app replaces it. |
...
### Data-mule bridge (`develop` backend `POST /ingest`)
